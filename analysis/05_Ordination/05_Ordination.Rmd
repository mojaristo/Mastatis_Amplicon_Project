...
- For this dataset, I will scale to the nearest threshold above the lowest high quality sample which is 8,642. Thus i will scale to **8,500**
- Scaling to 8,500 reads helps reduce bias caused by unequal library sizes, making differences among samples more reflective of biological composition rather than sequencing effort
...
- Bray–Curtis PERMANOVA found no statistically significant differences in overall rumen microbial community composition among groups J, L, and Y (R² = 0.043, p = 0.129).
- roup identity explained only a small proportion of total beta-diversity variation, suggesting that most microbiome differences occur among individual samples rather than among groups.
- I fail to reject the null hypothesis and conclude that the three groups do not possess strongly distinct average microbial communities.
...
#Post-hoc test: to see which grouo is most dispersed 
boxplot(dispr_sorensen_salGroup)
TukeyHSD(dispr_sorensen_salGroup)
```

Interpreting `betadisper()/permutest()` results:

**Sorensen:**
- Sorensen distances found a significant difference among groups (F = 5.56, p = 0.006). This indicates that the spread of microbial communities differs across groups J, L, and Y.
- Therefore, some groups are more compositionally heterogeneous than others. This suggests that group-level beta-diversity differences may reflect variation in within-group dispersion rather than strong separation of community centroids alone
...
```{r betadisper-bray}
# Bray-Curtis Betadisper - Group (Categorical)  
dispr_bray_salGroup <- betadisper(scaled_bray_dist, metadata_df$Group)
permutest(dispr_bray_salGroup)

#Post-hoc test: to see which grouo is most dispersed 
boxplot(dispr_bray_salGroup)
TukeyHSD(dispr_bray_salGroup)
```

**Bray-Curtis:**
- Bray-Curtis distances detected significant differences among groups (F = 5.20, p = 0.008). This indicates that the variability of abundance-weighted microbial community composition differs across groups J, L, and Y.

### Comparing Sorensen & Bray Curtis Results 

- Both metrics produced consistent overall conclusions. There was substantial overlap among groups J, L, and Y, indicating that the three groups do not form strongly distinct microbial community clusters.
- Group differences are driven more by heterogeneity than by clear centroid separation since homogeneity of dispersion tests were significant for both Sorensen (p = 0.006) and Bray–Curtis (p = 0.008)
- differences among groups likely involve both taxa occurrence and abundance variation rather than one process alone.This is because Sorensen is based on presence–absence, so it reflects whether ASVs are shared among groups. Bray–Curtis includes relative abundance, so it is sensitive to shifts in dominant and common taxa.
...
*Bray–Curtis PERMANOVA detected no significant difference in average microbial community composition among groups J, L, and Y (R² = 0.043, p = 0.127), indicating that group membership explains only a small proportion of total beta-diversity variation. In contrast, Bray–Curtis betadisper analysis was significant (F = 5.20, p = 0.010), demonstrating unequal within-group dispersion. Pairwise comparisons showed that groups L and Y were significantly more dispersed than group J. Together, these results suggest that the primary group-level signal is not complete compositional separation, but greater microbiome heterogeneity in groups L and Y relative to the more compositionally stable group J.*
...
- This suggests that the core rumen microbiome may be relatively similar across all groups, but the stability or consistency of that microbiome differs. 
- Group J appears to represent a more uniform microbial state, while groups L and Y may reflect more variable ecological conditions, disease states, dietary influences, or other factors that increase microbiome heterogeneity.

*It is difficult to tell because I do not have the exact metadata labels, therefore my original idea that groups J, Y and L respond to disease state may likely be incorrect*
